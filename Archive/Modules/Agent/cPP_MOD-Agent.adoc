...
[#fig-fco-cpc-lifecycle]
.Illustrative FCO_CPC_EXT.1 communication-pairing lifecycle
image::fco-cpc-lifecycle.svg[Administrative pairing enablement, registration, permitted communication, and disablement,pdfwidth=100%,align=center]
...
*Application Note {counter:appnote}:* A registration channel protected according to FTP_DIT_EXT.1 may be used over any untrusted network. This PP-Module makes no assumption that the registration network is trusted.
...
[loweralpha]
. Enabling a communication pairing as in FCO_CPC_EXT.1.1, including each endpoint identity and whether it represents a set of one or more TOE Component Instances sharing a TOE Component identity or one separately managed TOE Component Instance.
. Disabling a communication pairing as in FCO_CPC_EXT.1.3, including the endpoint identities and whether the pairing or an endpoint identity was disabled.
...
*FCO_CPC_EXT.1.2* The TSF shall implement a registration process in which TOE Component Instances represented by an enabled communication pairing establish and use a communications channel that uses [_selection:_
