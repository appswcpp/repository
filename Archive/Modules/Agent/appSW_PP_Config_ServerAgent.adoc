...
[%unbreakable]
--
The ST shall provide an SFR allocation rationale that identifies the allocation category and any feature-dependent condition for each claimed requirement: all TOE Components, applicable TOE Components that perform the relevant function, at least one TOE Component, or an allowed operational environment dependency. The ST shall describe all distinct intra-TOE Communication Relationship Types and identify the mechanisms used to authorize and protect those communications. Repeated relationships may be represented using an unambiguous topology pattern rather than by enumerating every TOE Component Instance pair. Communication between TOE Component Instances that uses a network interface or network protocol stack is treated as transmitted data for FTP_DIT_EXT.1 regardless of whether the route is loopback, same-host virtual networking, container networking, a LAN, or a WAN. For non-network local inter-process communication, such as named pipes, Unix domain sockets, shared memory, platform-brokered IPC, or equivalent mechanisms, the ST shall identify the mechanism and the architectural protection relied upon, such as platform isolation, endpoint access control, or object permissions. Detailed implementation or configuration information is required only where an applicable SFR or Evaluation Activity requires it.
--

==== SFR Allocation for Distributed TOEs
For a distributed TOE, the SFRs are satisfied by the TOE as a whole; however, not every SFR is necessarily implemented by every TOE Component. The ST author shall use the following allocation categories to identify how each SFR is satisfied by the distributed TOE. Every deployed TOE Component Instance shall exhibit the claimed SFR behavior allocated to its TOE Component in its identified configuration; deployment multiplicity does not permit one instance to satisfy an allocated requirement on behalf of another instance.
...
The requirement applies only when the feature, selection, or objective requirement is claimed for the distributed TOE. This condition does not itself allocate the requirement or reduce evaluation coverage. When a feature-dependent requirement is claimed, the allocation in the applicable table controls which TOE Components have concrete implementation obligations. The ST shall identify the TOE Component or components that perform the relevant function, make the relevant selection, or rely on the relevant platform functionality.
...
The allocation category and any feature-dependent condition are controlled by the following tables and the SFR's operations and application notes; the ST shall not freely reclassify an SFR. _At Least One Component_ and _Operational Environment Dependency_ may be used only where the applicable table or requirement expressly permits that category. Allocation shall be stated at the SFR-element level when different elements have different responsible TOE Components, update actors, or permitted platform dependencies. Evaluation evidence shall cover every TOE Component, artifact, communication path, or function allocated as responsible. Evidence for one TOE Component may be reused only where the ST demonstrates that the relevant implementation, configuration, and execution context are equivalent.

===== Non-Normative Example: Conditional Update Selection and Component Coverage

Consider a distributed TOE containing a Server Application image and an Agent Application package. The trusted-update selection in FPT_TUD_EXT.2 is feature dependent: if the selection is not claimed, the requirement is not applicable. If the selection is claimed, the table's _Applicable Components_ allocation applies. The ST identifies every TOE Component that performs update verification, delivery, replacement, or installation integrity functions, together with its artifact and path.

For example, the Server Application may verify and install the Server image while the Agent Application verifies and installs the Agent package. The ST allocates the relevant FPT_TUD_EXT.2 elements to those responsible TOE Components. The Server evidence need not prove the Agent package verification or installation element, and the Agent evidence need not prove the Server image element; together, the evidence covers all allocated elements. A passing Server-image test, common registry, build pipeline, base image, or package format does not by itself satisfy the Agent update path unless the ST demonstrates equivalent security-relevant implementation, configuration, and execution context. This example does not alter the requirement or its Evaluation Activities.
...
|FTP_DIT_EXT.1
|Applicable Components
|Applies to each TOE Component that transmits data or sensitive data to another trusted IT product or TOE Component Instance, or invokes platform-provided functionality where the SFR permits that protection. Protection of network-mediated intra-TOE communications between TOE Component Instances is addressed by FTP_DIT_EXT.1 in the base cPP, including loopback, same-host virtual networking, container networking, LAN, and WAN paths. If an operational-environment ingress, proxy, or mesh terminates a connection, the payload-side TOE interface and environmental dependency remain in scope; the public-facing interface is TOE behavior only when the terminating component is in the TOE boundary. Non-network local inter-process communication shall be identified in the ST and mapped to the applicable base cPP controls.
|===
...
Runtime Replicas do not create additional Communication Relationship Types solely because there are more connections. Additional coverage is required when an instance has a separately managed identity, trust configuration, interface, placement, failover path, or other difference that can affect the result.
