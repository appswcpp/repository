...
A _Feature Dependent_ SFR or SFR element is fulfilled only where the relevant feature is implemented by a distributed TOE Component. If a _Feature Dependent_ SFR or element is included in the ST, at least one TOE Component shall implement the relevant feature. The ST shall map every TOE Component that implements the feature to the SFR element or elements it implements. Where different TOE Components implement different elements of a multi-element SFR, each TOE Component shall satisfy only its mapped elements; the mapped elements collectively satisfy the TOE-level requirement. A feature, selection, or objective claim that controls inclusion of the SFR shall be recorded separately in the ST mapping and shall not be used to omit an implementing TOE Component or reduce required coverage.
...
The claim condition, where relevant, and component allocation are controlled by the following tables and the SFR's operations and application notes; the ST shall not freely reclassify an SFR. _At Least One Component_ and a permitted operational-environment contribution may be used only where the applicable table or requirement expressly permits them. Allocation shall be stated at the SFR-element level when different elements have different responsible TOE Components, update actors, or permitted platform dependencies. A _Feature Dependent_ allocation or claim condition shall not be used to omit a TOE Component that performs the mapped function. Evaluation evidence shall cover every TOE Component, artifact, communication path, or function allocated as responsible. Evidence for one TOE Component may be reused only where the ST demonstrates that the relevant implementation, configuration, and execution context are equivalent.

[[fig-allocation-coverage-guardrail]]
.Illustrative claim, allocation, mapping, and retained Evaluation Activity coverage
image::allocation-coverage-guardrail.svg[Claim condition, component allocation, ST mapping, evidence reuse, and retained coverage,pdfwidth=100%,align=center]

===== Rationale: NDcPP-Aligned Feature Dependent Allocation

This configuration uses the _All Components_, _At Least One Component_, and _Feature Dependent_ allocation terms used by the NDcPP distributed-TOE model. A claim condition, where relevant, explains whether a feature, selection, or objective brings an SFR into the ST. _Feature Dependent_ then identifies the TOE Components in which the claimed behavior is implemented. The ST records both facts in its mapping, but they are not separate allocation categories.

The required element-level mapping prevents a coverage shortcut. A TOE Component that does not implement an element is not treated as its implementer; conversely, a component that does implement it cannot be omitted merely because another component implements a similar function. The TOE as a whole must still satisfy every claimed SFR. Evidence for one component can be reused only when equivalent security-relevant implementation, configuration, and execution context are demonstrated; shared infrastructure, a common base image, or similar function names are not by themselves equivalent.
...
* the component allocation: _All Components_, _Feature Dependent_, or _At Least One Component_;
...
Consider a distributed TOE containing a Server Application image and an Agent Application package. The trusted-update selection in FPT_TUD_EXT.2 is the claim condition: if it is not claimed, the requirement is not included in the ST. If it is claimed, the requirement uses the _Feature Dependent_ allocation, and every TOE Component that performs update verification, delivery, replacement, or installation-integrity functions fulfills its mapped SFR element. The ST identifies each such TOE Component together with its artifact and path.

In plain terms, a successful test of the Server image does not prove that the Agent package is protected. If the Server verifies and installs its own image while the Agent verifies and installs its package, each component is mapped only to the update elements it performs. The Server evidence need not prove the Agent's element, and the Agent evidence need not prove the Server's element; together, the evidence must cover all allocated elements. A common registry, build pipeline, base image, or package format does not by itself make the two paths equivalent. This example does not alter the requirement or its Evaluation Activities.
...
|FCO_CPC_EXT.1
|Feature Dependent
|The ST shall map FCO_CPC_EXT.1.1 (enablement), FCO_CPC_EXT.1.2 (registration), and FCO_CPC_EXT.1.3 (disablement) to the TOE Component or Components that implement them. A TOE Component that provides an enablement or disablement function, and a TOE Component that performs registration, shall satisfy only its mapped element or elements; the mapped elements collectively satisfy FCO_CPC_EXT.1. The ST shall identify the affected endpoint identities and Communication Relationship Types.
|===
...
[[fig-twelve-container-topology]]
.Illustrative twelve-container TOE topology and Communication Relationship Types
image::twelve-container-topology.svg[Five TOE Components, twelve deployed TOE Component Instances, and relationship types in the non-normative worked example,pdfwidth=100%,align=center]
...
|All Components (all five in this example)
...
|Feature Dependent
|FCO_CPC_EXT.1 is mapped to the TOE Components assigned an Agent Application role, and FTP_DIT_EXT.1 is mapped to the transmitting TOE Components. The ST maps each Communication Relationship Type below to the applicable iteration and retains relationship-, identity-, and channel-specific testing.
