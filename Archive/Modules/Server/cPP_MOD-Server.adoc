...
For a distributed TOE, the ST shall identify each TOE component type, describe the role of each component type, identify which component types implement each claimed SFR, and describe all distinct communication relationship types between TOE components. Repeated relationships may be represented using an unambiguous topology pattern rather than by enumerating every runtime component pair. The ST shall also distinguish TOE components from operational environment components. Operational environment components may include container orchestration, container runtimes, host operating-system kernels, platform-provided operating-system services or userland not delivered as part of a TOE image, service mesh infrastructure, ingress infrastructure, cluster networking, platform-provided secret or configuration stores, managed runtimes, application frameworks, and other infrastructure services not explicitly included in the TOE boundary.
...
*Application Note {counter:appnote}:* This requirement is designed to ensure that platform security features do not need to be disabled in order for the application to run. The assignment in FPT_AEX_EXT.1.3 in the {base-pp} must be "no exceptions". A Server Application payload executing in a Linux container uses the Linux case. The relevant Linux host or kernel security control, such as SELinux or AppArmor, shall remain enabled and enforcing and the evaluated runtime or orchestrator profile shall be applied; those configuration and dependency checks are assessed separately from the running-payload execution.
