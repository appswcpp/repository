...
=======
Read and write access to the TOE’s configuration data shall be limited to Administrator, TOE and [assignment: list of authorized entities]
...
Application Note {counter:appnote}: Functions that relate to management of agents is intended to be used in conjunction with the Agent module, however, it can be used with third party agents that aren't necessarily within the TOE boundary.
