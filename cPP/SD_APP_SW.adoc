...
|1.0e
|2024-02-15
|Incorporated feedback recevied following initial release.
...
The evaluator shall review the TSS to determine that it describes how the functionality described by FCS_RBG_EXT.1 is invoked.  The TSS shall identify the key generation algorithms, key sizes, and usage.
...
If asymetric keys are generated, The evaluator shall ensure that the TSS identifies the key sizes supported by the TOE. If the ST specifies more than one scheme, the evaluator shall examine the TSS to verify that it identifies the usage for each scheme.
...
===== Evaluation Activity
The evaluator shall examine the Operational Guidance to ensure that the following ALC_FLR.3 information is included:
...
ALC_FLR.3 documentation is split into two inputs: guidance for TOE users and procedures for TOE developers.  The ALC_FLR.3 guidance must be a public facing document.  The ALC_FLR.3 procedures may be a public or private document. 
