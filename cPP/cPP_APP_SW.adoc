...
For containerized applications the container is treated as the platform and as such should be described in the TSS. Any assets that are not inherited from the Guest OS namespace that are packaged with the container definition are within the TOE Boundary.
